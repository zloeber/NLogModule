...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        ModuleLicenseURI = 'https://github.com/zloeber/nlogmodule/LICENSE.md'
        ModuleTags = 'nlog, logging, wrapper' -split ','
        ModuleAuthor = 'Zachary Loeber'
        ModuleDescription = 'NLog PowerShell wrapper'
        ...
        # You should not actually enter this key here but should manually enter it in the NLogModule.buildenvironment.json file
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "NLogModule.buildenvironment.json"
